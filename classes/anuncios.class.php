...
class Anuncios 
{
	...
	public function getTotalAnuncios($filtros){
		...
		$sql = $pdo->prepare("SELECT count(id) as total FROM anuncios WHERE ".implode(' AND ',$filtrostring)."");
	...
	public function addAnuncio($usuario,$titulo,$descricao,$categoria,$valor,$estado){
		global $pdo;
		$sql = $pdo->prepare("INSERT INTO anuncios set titulo=:titulo, id_usuario=:usuario, descricao=:descricao, id_categoria=:categoria, valor=:valor, estado = :estado");
	...
	public function excluirAnuncio($id,$usuario){
		...
		$sql = $pdo->prepare("DELETE FROM anuncios where id_usuario=:id_usuario and id=:id");
	...
	public function getAnuncio($id,$usuario){
		global $pdo;
		$array = array();
		$sql = $pdo->prepare("SELECT * FROM anuncios where id=:id and id_usuario=:id_usuario");
		$sql->bindValue(":id",$id);
		$sql->bindValue("id_usuario",$usuario);
		$sql->execute();
	...
	public function editarAnuncio($usuario,$titulo,$descricao,$categoria,$valor,$estado,$fotos,$id){
		global $pdo;
		$sql = $pdo->prepare("UPDATE anuncios set titulo=:titulo, id_usuario=:usuario, descricao=:descricao, id_categoria=:categoria, valor=:valor, estado = :estado where id=:id");
		$sql->bindValue(":titulo",$titulo);
		$sql->bindValue(":usuario",$usuario);
		$sql->bindValue(":descricao",$descricao);
		$sql->bindValue(":categoria",$categoria);
